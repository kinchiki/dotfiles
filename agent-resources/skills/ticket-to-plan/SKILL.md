...
引き継ぎ先は Claude Desktop のサイドバーに新しいセッションとして表示される形にします。
引き継ぎ prompt は self-contained にし、absolute plan-file path、source reference、working directory を含めます。
source reference が ticket ではない場合は、source summary と plan-id を含めます。

`spawn_task` が使える場合はそれを使ってください。
チップをクリックすると新しい worktree で実装セッションが起動し、Claude Desktop のサイドバーに表示されます。
...
`spawn_task` が使えない場合は、Claude Desktop で新しい会話を開き、working directory を repo に設定して、次の self-contained prompt を貼り付けてもらってください。
新しい会話は Claude Desktop のサイドバーにセッションとして表示されます。
terminal の CLI コマンドは案内しないでください。

```text
プラン <absolute-plan-file-path> を implement-plan スキルで実装して
```

最後に、plan の保存先と implementation session の起動方法（`spawn_task` チップ起動済みか、サイドバーで新規会話を開いて貼り付けるか）を報告して停止してください。
...
- implementation session を起動したか（`spawn_task` チップ）、または Claude Desktop で新規会話を開いて prompt を貼り付ける必要があるか
