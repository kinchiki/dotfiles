...
```bash
cd <repo> && claude "プラン <absolute-plan-file-path> を implement-plan スキルで実装して"
```
...
| 1 | Resolve source context | `gh` CLI / GitHub MCP / Linear MCP / user request. |
| 2 | Research codebase | Read-only planning. |
| 3 | Break into tasks | `files`, `depends_on`, `parallel`, `test`, `done_when`. |
| 4 | Cross-AI planning review | Read `references/planning-ai-review.md`. |
| 5 | Get approval | Present reviewed plan and review disposition. |
| 6 | Write plan file | Read `references/plan-template.md`. |
| 7 | Hand off implementation | `spawn_task` or manual command. |
