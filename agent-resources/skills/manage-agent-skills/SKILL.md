...
作成または更新した skill は、規定の symlink で `.agents/` と `.claude/` の両方へ公開してください。
コマンドと検証手順は `references/repository-links.md` を使ってください。
既存 path が意図した symlink ではない場合は、置き換える前に停止して確認してください。

#### Agent definition links

このリポジトリでは agent definition 実体を `agent-resources/agents/<agent-name>/` に置きます。
作成または更新した agent definition は、規定の symlink で `.agents/`、`.claude/`、`.codex/` へ公開してください。
コマンドと検証手順は `references/repository-links.md` を使ってください。
既存 path が意図した symlink ではない場合は、置き換える前に停止して確認してください。
