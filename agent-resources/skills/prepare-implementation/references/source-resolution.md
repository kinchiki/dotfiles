...
要約の内容を design.md の `## 要件` に書き、受入基準には `AC1` から ID を付ける。

入力が薄い、矛盾している、または受入基準が不足する場合は、その gap を扱う先を次のように分ける。

- 要件そのもの（対象の特定、受入基準、明らかな矛盾）が決まらないと調査を始められない gap は、`../../ask-user-questions/SKILL.md` に従ってユーザーへ確認し、回答を `## 要件` に反映する。
- 複数の選択肢があり、設計と一緒に決めるべき gap は、Step 2 で設計判断の候補として grill-design へ渡す。
- 計画の成否に影響しない gap は、`## リスク` に書く。
