...
- Use the relevant linter as the source of truth for linter-backed findings before treating them as blocking.
- For RuboCop `Layout/LineLength`, run the repo's RuboCop command with `--only Layout/LineLength`; if no repo wrapper exists, run `rubocop --only Layout/LineLength`.
- For multibyte text such as Japanese, base line-length decisions on the linter result instead of `awk`, byte counts, or byte-based editor measurements.
- If the relevant linter is green, record the finding as a false positive and leave the code unchanged for that finding.
