...
description: >-
  ...
  対応実装後に commit / push、PR description 更新、対象レビューコメントへの対応済み reply、thread resolve まで行う。
---
...
- 承認済み review 対応の実装、quality gate、local commit、push、PR description 更新、対象 thread への reply、thread resolve まで扱う。
- Review summary body は実装対象に含めるが、inline thread ではないため reply / resolve の対象外にする。

## Resources

- `references/github-review-thread-commands.md`: Step 1 と Step 8 で GraphQL 取得、reply、resolve の command が必要になったら読む。
...
- PR description は実装後の状態に合わせて更新し、実装と矛盾しない既存内容は保持する。
...
取得 command は `references/github-review-thread-commands.md` を使ってください。
Review summary body も同じ reference に従って取得してください。
...
- `gh pr view` で現在の PR description body を取得する。
- PR description は読み手が最終実装を理解するために更新する。
- 既存の要件、検証手順、リリースノート、スクリーンショットなどが実装後の状態と矛盾する場合は、該当 section を実装に合わせて更新する。
- 非自明な設計判断、意図的に見送った項目、レビュワーが再確認すべき変更がある場合だけ、短い note として追加する。
- 実装と矛盾しない既存 section は変更しない。
...
- reply / resolve command は `references/github-review-thread-commands.md` を使う。
