...
- `agent-resources/skills/open-pr/references/pr-body-template.md`: 現在の PR body が空で、repo 固有 template も見当たらない場合だけ読む。
...
- 現在の実装差分、直近の commit、lint / test 結果、ユーザーが明示した補足を集める。
- PR body にすでに書かれている内容と、現在の実装で変わった点を比較する。
- repo 固有の PR template や section 構成が見える場合は、それを維持する。
- 現在の PR body が空で、repo 固有 template も見当たらない場合だけ `agent-resources/skills/open-pr/references/pr-body-template.md` を読む。
