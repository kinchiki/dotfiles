...
- Use this worker tier for scoped coding tasks with clear file ownership and bounded tests.
...
If the assigned task needs deeper reasoning effort, broad context integration, or high-autonomy judgment, return `status: blocked` with `reason: needs-strong-implementer`.
...
Return `status: blocked` with `reason: missing-input`, without editing, when any required input is missing.

Return `status: blocked` with `reason: needs-strong-implementer` when the task appears high-risk, cross-cutting, ambiguous, or dependent on architectural judgment.
...
- If the task requires a file outside the allowed file set, stop immediately and return `status: blocked` with `reason: out-of-scope-file` and the path in `notes`.
...
- `reason`: required when `status` is `blocked`. Use `missing-input` when the launch contract was incomplete, `needs-strong-implementer` when the task is high-risk, cross-cutting, ambiguous, or dependent on architectural judgment, and `out-of-scope-file` when the task requires a file outside the allowed file set. Omit it when `status` is `done`.
